...
//         const tdee = getTDEE();
//         const prompt = alimentos.length > 0
//             ? `Crea un plan de comidas para una persona cuyo objetivo es ${userProfile.objetivo} y tiene un TDEE de ${tdee} kcal. Los alimentos disponibles son: ${alimentos.join(', ')} . Te comunicas de manera formal y detallada para asegurar claridad y precisión en la información proporcionada. Evita hablar de más por tema de tokens. Mándame justamente la tabla de la dieta y algún comentario de por medio refiriendose a los macros de cada comida , pero no te excedas. Por favor, devuelve la respuesta SOLO en formato HTML para que se vea bonita en una página web, no necesito texto introductorio.Mandame desde el <doctype> y empieza con el body</doctype>`
//             : `Crea un plan de comidas para una persona cuyo objetivo es ${userProfile.objetivo} y tiene un TDEE de ${tdee} kcal. Devuelve la respuesta en formato HTML para que sea fácil de leer y bonita.  Devuelve la respuesta en formato HTML para que sea fácil de leer y bonita.Solo pasame el html, ya que estoy utilizando la api y voy a consumir su informacion directamente y mostrarlo por pantalla.`;
...
//             const mealText = data.choices[0].message.content;
//             setMeal(mealText);
//             const savedMeals = JSON.parse(localStorage.getItem('meals')) || [];
//             localStorage.setItem('meals', JSON.stringify([...savedMeals, mealText]));
//         } catch (error) {
